...
export default {
  ...
  created() {
    import(`@views/Presell/${this.isMobile() ? "Mobile" : "Client"}/module_1.vue`).then(
      (res) => {
        this.moduleOne = res.default;
      }
    );
    import(`@views/Presell/${this.isMobile() ? "Mobile" : "Client"}/module_2.vue`).then(
      (res) => {
        this.moduleTwo = res.default;
      }
    );
    import(`@views/Presell/${this.isMobile() ? "Mobile" : "Client"}/module_3.vue`).then(
      (res) => {
